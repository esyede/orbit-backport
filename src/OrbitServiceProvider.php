...
class OrbitServiceProvider extends ServiceProvider
{
        ...
        $this->app->singleton(OrbitManager::class, function ($app) {
            ...
            foreach ($drivers as $key => $driver) {
                ...
                if (! in_array(Driver::class, $implements)) {
                    throw new Exception(
                        '[Orbit] The '.$driver.' driver must implement the '.
    ...
    public function boot()
    {
        ...
        Blueprint::macro('hasColumn', function ($name) {
            return collect($this->getColumns())->contains(function (ColumnDefinition $column) {
                    return $column->get('name') === $name;
            });
        });

        Blueprint::macro('orbitGetColumn', function ($name) {
