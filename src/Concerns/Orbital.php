...
trait Orbital
{
    ...
    public function migrate()
    {
        ...
        static::resolveConnection()
            ->getSchemaBuilder()
            ->create($table, function (Blueprint $table) use (&$blueprint) {
                static::schema($table);
        ...
        $driver->all(static::getOrbitalPath())
            ...
            ->each(function (Collection $chunk) {
                return static::insert($chunk->toArray());
            });
    }
    ...
    protected static function ensureOrbitDirectoriesExist()
    {
        ...
        $fs = new Filesystem();
        $fs->ensureDirectoryExists(static::getOrbitalPath());
        $fs->ensureDirectoryExists(config('orbit.paths.cache'));
    ...
    public static function generateOrbitalFilePathForModel(Model $model)
    {
        ...
        $path = static::getOrbitalPath()
            . DIRECTORY_SEPARATOR
            . Support::buildPathForPattern($pattern, $model);
